...
app.post('/api/operations', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    const { operationCode, containerNumber, originPort, originCountry, destinationPort, destinationCountry, weightKg, incoterm, clientName, shippingLine, costEstimate, priority } = req.body
    
    const operation = await prisma.operation.create({
      data: {
        operationCode, containerNumber, originPort, originCountry, destinationPort, destinationCountry,
        weightKg, incoterm, clientName, shippingLine, costEstimate, priority,
        userId: req.userId!,
